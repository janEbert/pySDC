...
from pySDC.projects.parallelPFASST.div_linearized_implicit_parallel_MPI import div_linearized_implicit_parallel_MPI
from mpi4py import MPI

class div_linearized_implicit_fixed_parallel_MPI(div_linearized_implicit_parallel_MPI):
    """
    ...
    def __init__(self, params):
        ...
        super(div_linearized_implicit_fixed_parallel_MPI, self).__init__(params)
    ...
    def update_nodes(self):
        ...
        #        Guv.append(P.dtype_u(P.init, val=0))#.append(np.zeros( Gu_[m].values.size, dtype=complex))
        #    else:
        #        Guv.append(P.dtype_u(P.init, val=0))#.append(None) 


        #for m in range(M):                
        #    if m in self.node_list:
        #        for j in range(M):
        #            Guv[m] += self.Vi[m, j] * Gu_[j]


        Guv = []
        for m in range(M):
            if m in self.node_list:
                Guv.append(np.zeros( Gu_[m].values.size, dtype=complex))
            else:
                Guv.append(None) 
        ...
        for m in range(M):
            ...
            if m in self.node_list:
                ...
                #Guv[m] = Guv[m].reshape((np.sqrt(Guv[m].size)).astype(int) , (np.sqrt(Guv[m].size)).astype(int) )   #ruth                       
                Guv[m] = Guv[m].reshape(2, (np.sqrt(Guv[m].size/2)).astype(int) , (np.sqrt(Guv[m].size/2)).astype(int) )   #ruth       
            else:
                for j in self.node_list:
                    U = U + (self.Vi[m,j] * Gu_[j].values).flatten()  
                root=0    
        ...
        for m in self.node_list: # range(M):  # hell yeah, this is parallel!!
            ...
            #uv_g[m].values = (P.solve_system_jacobian(dfdu, Guv[m].values, L.dt * self.D[m], L.u[0], L.time + L.dt * self.coll.nodes[m]).values)
        ...
        for m in range(M):
            ...
            if m in self.node_list:
                ...
                #L.u[m + 1].values += K.reshape((np.sqrt(Guv[m].size)).astype(int) , (np.sqrt(Guv[m].size)).astype(int) )   #ruth     
                #
                L.u[m + 1].values += K.reshape(2, (np.sqrt(Guv[m].size/2)).astype(int) , (np.sqrt(Guv[m].size/2)).astype(int) )   #ruth     
            else:
                for j in self.node_list:
                    U = U + ((self.V[m,j] * uv_g[j].values.flatten()).astype(float))#.flatten()  
                root=0    
        ...
        #for m in range(M):
            ...
            #else:
                #root=self.proc[m]
            #uv_g[m].values = self.params.comm.bcast([uv_g[m].values,MPI.COMPLEX], root=root)
        #for m in range(M):
            #for j in range(M):                  
                #L.u[m + 1].values += ( self.V[m, j] * uv_g[j].values[0]).astype(float)   #.astype(float, casting='unsafe') 
