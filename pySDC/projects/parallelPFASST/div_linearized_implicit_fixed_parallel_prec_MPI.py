    ...
    def update_nodes(self):
        ...
        for m in range(M):
            ...
            if m in self.node_list:
                ...
                Guv[m] = Guv[m].reshape(2, (np.sqrt(Guv[m].size/2)).astype(int) , (np.sqrt(Guv[m].size/2)).astype(int) )    
        ...
        for m in range(M):
            ...
            if m in self.node_list:
                ...
                L.u[m + 1].values += K.reshape(2,(np.sqrt(Guv[m].size/2)).astype(int) , (np.sqrt(Guv[m].size/2)).astype(int) )       
            else:
                for j in self.node_list:
                    U = U + ((self.V[m,j] * uv_g[j].values.flatten()).astype(float))#.flatten()  
                root=0    
